...
const NumberedList = ({ block, level }: { block: BaseBlock, level: number }) => {
    let children = null;
    if (block.children) {
        children = (
            <ol>
                {block.children.map((block: BaseBlock, i: number) => <NumberedList block={block} level={level + 1} key={`${level}-${i}`} />)}
            </ol>
        )
    }
